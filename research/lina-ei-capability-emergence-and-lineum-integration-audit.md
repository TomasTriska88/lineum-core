...
**Core base before this revision:** `080cd95a96018677ee985aed87afe4322ec70b20`  
...
- **0.4.0:** makes the audit self-reproducing. It embeds the complete P1A executable harness, independent verifier, exact machine-readable receipt, captured outputs, and hashes directly in this Markdown file. It also freezes a complete P1B reference implementation and test suite, records its isolated `3 passed` result, and explicitly separates that reference result from any claim that the private Lina product has been repaired.
...
The harness creates `p1a_receipt.json` and disposable per-case state files under `run/`. The exact expected outputs are embedded below.
...
### 12.3 Exact captured harness stdout

The absolute receipt path is environment-specific; all other values are deterministic for the frozen harness.
...
### 12.4 Exact generated `p1a_receipt.json`
...
**Status:** complete at `contract_reproduced` level. Full reproduction code and outputs are embedded in this audit.
...
- Core branch before revision: `develop` at `080cd95a96018677ee985aed87afe4322ec70b20`.
