...
The test-first `Q2-PV1-A` harness is now frozen as ordinary readable research code. Its pure unsupported-runtime preflight passed `7 / 7`; that test pass verifies helper logic only and is not the official scientific audit. The retained runner reproduces the canonical 5000-step primary path, applies no recovery steps, preserves the canonical `active` semantics, records all 28 case keys, independently exposes direct and analytic perturbation-ledger deltas, and fails closed on incomplete case sets or analytic/direct disagreement. The official supported-runtime result remains pending.
...
| Next consequential step | `Q2-PV1`: verify the canonical perturbation's effect on the declared `sum(|psi|^2) + sum(phi)` ledger across all 28 frozen pre-states before selecting or judging a conserving repair |
...
runner intended Git blob = e3657119b855965b4fd622b3e94f08443a7c9107
runner bytes = 13,146
...
test intended Git blob = 403c1cb8747cebf3280009b3b2ffcd814c72e060
test bytes = 3,214
...
Because the supported repository dependency range requires NumPy `<2.0` and the active local environment cannot supply it, the official run remains gated on the already-declared narrow GitHub-hosted exception. The official command may execute only after the runner/test source is committed on `develop`, the supported environment is verified, and the canonical localized plus `Q2-PV1` tests pass. No workflow artifact or ZIP may be uploaded.
...
The `Q2-PV1-A` runner was then written test-first. Before the runner existed, the new test collection failed with the expected missing-file error; no scientific case ran. After implementation, seven pure helper tests passed locally under NumPy `2.3.5`, and syntax compilation passed. These are pre-execution development checks only; the official supported-runtime 28-case audit remains unexecuted at version `0.21.2`.
...
25. the `Q2-PV1-A` runner and permanent pure regression tests were written test-first, passed `7 / 7` pure helper tests in the unsupported local runtime, and were frozen as ordinary readable research files before official supported-runtime execution.
