...
**Status:** active authoritative report; localized-L1 negative independently verified; `Q2-O1` observer-frame test completed; `Q2-SA1-A` independently reproduced in a repository-supported runtime; all eight Stage A cases are `unpaired_source_dominated`; no Stage B candidate survives; the owner authorized continued work under the hard three-question scope without proposing a new independent mechanism; replacement-mechanism selection remains uncommitted; `Q2-PV1` perturbation-ledger validity audit is preregistered; its exact test-first runner/test are now branch-reachable and read back at the frozen blob identities; official supported-runtime execution is pending  
**Version:** 0.21.5  
**Evidence cutoff:** 2026-08-07  
**Repository / branch:** `TomasTriska88/lineum-core` / `develop`  
**Checkpoint parent:** `52766474219b0ab7664e6f7925ee24d7a92f45e0`  
...
Before selecting any replacement equation, exact source inspection exposed a more basic Q2 validity question: the canonical radial perturbation multiplies central `psi` by `1.5` and annular `psi` by `0.5` while leaving `phi` unchanged. Therefore the perturbation does not, by construction, guarantee preservation of the implementation-defined `sum(|psi|^2) + sum(phi)` ledger already used by the Q2 accounting work. A strictly ledger-conserving repair could otherwise be judged partly by whether it returns to a state lying on a different ledger surface. `Q2-PV1` is preregistered to audit this methodological issue before any repair is selected. This does not retract the already verified Stage A result about the current implementation; it only protects the next repair comparison from a potentially incompatible perturbation.

The exact test-first `Q2-PV1-A` runner/test byte streams are now branch-reachable. Runner commit `9975ff4c6afef8d92f60f26b790a701a36820d65` published Git blob `e3657119b855965b4fd622b3e94f08443a7c9107`; test commit `52766474219b0ab7664e6f7925ee24d7a92f45e0` published Git blob `403c1cb8747cebf3280009b3b2ffcd814c72e060`. Both were read back exactly before this report checkpoint. No supported-runtime `Q2-PV1-A` audit has yet executed.
...
| Next consequential step | `Q2-PV1-A`: run the supported-runtime frozen test gate and 28-case perturbation-ledger audit now that the exact runner/test are branch-reachable; retain the primary before selecting or judging any conserving repair |
...
Because the supported repository dependency range requires NumPy `<2.0` and the active local environment cannot supply it, the official run remains gated on the already-declared narrow GitHub-hosted exception. After this `0.21.5` report checkpoint, the official command may execute only from the committed branch state, after the supported environment is verified and the canonical localized plus `Q2-PV1` tests pass. No workflow artifact or ZIP may be uploaded.
...
This report does not establish a universal galactic `tanh` law; autonomous `98%` emergence; absence of dark matter; modified gravity; a mathematical bridge to TOLOG; a physical `3 x 3` elementary cell; TOLOG Dark Heart; an emergent `tanh` replacement; a natural bound-free attractor; a complete `psi`–`phi` energy cycle; scalar-potential memory; a causal disk mechanism; a universal absence of moving or rotating frames; a universal spatial-accounting mechanism beyond the frozen Stage A domain; the correct replacement mechanism; ledger neutrality or non-neutrality of the canonical perturbation in the supported runtime before `Q2-PV1-A` executes; or a laboratory, quantum, gravitational, cosmological, consciousness, or ontological interpretation.
...
29. the exact `Q2-PV1-A` runner and test are now branch-reachable through commits `9975ff4c6afef8d92f60f26b790a701a36820d65` and `52766474219b0ab7664e6f7925ee24d7a92f45e0`; readback confirms the frozen blob identities exactly, and no scientific execution occurred between the sequential commits.
