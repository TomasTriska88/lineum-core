...
The official supported-runtime `Q2-PV1-A` primary now finds that all 28 canonical rows are active at perturbation time and that the direct array calculation agrees with the frozen analytic identity in every row. Under the frozen numerical neutrality criterion, `24 / 28` active perturbations are non-neutral and `4 / 28` are neutral. The four neutral rows are not ordinary finite-amplitude counterexamples: two have exactly zero `psi` after the no-interaction-denominator lane has reset, and two have `psi` totals around `10^-21`, so the absolute `1e-10` numerical floor classifies their tiny ledger shifts as neutral. This primary therefore supports the preregistered concern that the canonical perturbation is not universally ledger-neutral across the actually perturbed frozen states. It does **not** invalidate the earlier verified Q2 negative, does not make Q2 positive, and is not yet an independently verified methodological conclusion.

The retained primary also corrects the unsupported exploratory expectation that two rows would be inactive. The official supported-runtime active mask is `28 / 28` active. Exploratory NumPy `2.3.5` values remain hypothesis-generating only and are not promoted by similarity to parts of the primary.
...
2. **Reproduced observations:** homogeneous and localized independent checkers agree with their primaries; neither tested route demonstrates reciprocal full-state return; `Q2-O1` changes no case classification under the frozen frame and phase transforms; the supported-runtime Stage A primary and separately implemented Stage A checker now agree exactly on all frozen compared numbers and categories. All eight Stage A cases are `unpaired_source_dominated`, none is full-state recovery or `near_return`, and no Stage B candidate exists. The new `Q2-PV1-A` supported-runtime primary also reports direct/analytic agreement in all 28 rows and `24 / 28` active perturbations as ledger-non-neutral, but this new methodological finding is not yet independently verified.
...
| Next consequential step | independently rederive the retained `Q2-PV1-A` sufficient statistics and neutrality labels without importing the primary decision function; retain checker output before executing the already-preregistered ledger-neutral perturbation control |
...
**What is not yet established:** this methodological conclusion has not yet passed the separately implemented checker; the primary does not invalidate earlier Q2 negatives, identify a repair, or make Q2 positive.  
**Real-physics relation:** none established.
...
The independently reproduced Stage A result establishes only the bounded implementation-level statement that, under the frozen test conditions and declared accounting, the observed positive `psi` gain is dominated by unpaired gain rather than reciprocal incoming neighbour transport and does not produce full-state return. The new Q2-PV1 primary establishes only that one frozen supported-runtime execution produced 24 non-neutral and four numerically neutral active perturbation rows with complete analytic/direct agreement; its final methodological status awaits independent verification.
