...
**Status:** active authoritative report; localized-L1 negative independently verified; `Q2-O1` observer-frame test completed; `Q2-SA1-A` independently reproduced in a repository-supported runtime; all eight Stage A cases are `unpaired_source_dominated`; no Stage B candidate survives; the owner authorized continued work under the hard three-question scope without proposing a new independent mechanism; replacement-mechanism selection remains uncommitted; `Q2-PV1` perturbation-ledger validity audit is preregistered and its test-first runner/test bytes are frozen before supported-runtime execution, with branch publication still pending  
**Version:** 0.21.3  
**Evidence cutoff:** 2026-08-07  
**Repository / branch:** `TomasTriska88/lineum-core` / `develop`  
**Checkpoint parent:** `0caabdcdd97c7e11ac7cdc6a924f2b49e9d457bd`  
...
The exact test-first `Q2-PV1-A` runner/test byte streams are frozen and locally preflighted, but at this checkpoint they remain unreferenced Git staging objects rather than branch-reachable source. No supported-runtime audit has run. The next repository action is purely publication: create a fresh two-file commit on the current `develop` head using the already frozen runner blob `e3657119b855965b4fd622b3e94f08443a7c9107` and test blob `403c1cb8747cebf3280009b3b2ffcd814c72e060`, verify that the candidate changes exactly those paths, then fast-forward `develop`. Only after that publication and readback may the supported-runtime test gate execute.
...
| Next consequential step | `Q2-PV1`: publish the already frozen two-file audit harness on current `develop`, then verify the canonical perturbation's effect on the declared `sum(|psi|^2) + sum(phi)` ledger across all 28 frozen pre-states before selecting or judging a conserving repair |
...
A first candidate commit `afd0fb05970d91dadd7628033dcf2759d3f37aa0` and a later rebased candidate `e0992917c7f9693ea04fa3c3d7b0c649d59115f1` were created as unreferenced Git Data objects while the report was being corrected. Neither was moved onto `develop`; therefore neither is a published checkpoint and neither may be cited as branch state. No science executed from either candidate. Version `0.21.3` explicitly corrects the temporary `0.21.2` wording that described the harness as committed.

Because the supported repository dependency range requires NumPy `<2.0` and the active local environment cannot supply it, the official run remains gated on the already-declared narrow GitHub-hosted exception. The official command may execute only after the exact runner/test blobs become reachable from `develop`, the supported environment is verified, and the canonical localized plus `Q2-PV1` tests pass. No workflow artifact or ZIP may be uploaded.

## 9. Preserved failure and publication chronology

Technical non-results retained from history include a stalled checkout; five fail-closed archive-locator attempts (`31051766659`, `31052012351`, `31052264959`, `31052349446`, `31052454245`); one unretained full-screen recomputation that exceeded the command limit; one unretained migration replay; an accidental temporary branch; a checker workflow rejected before job creation; the pre-execution `Q2-O1` dynamic-import failure; a multi-repository code-search HTTP `502`; the first `Q2-SA1-A` checker comparison failure described in Section 8.13; and the first post-repair supported-runtime regression attempt whose synthetic fixture was inconsistent with the frozen disabled-stage lane map. None is scientific evidence.
...
The `Q2-PV1-A` runner was then written test-first. Before the runner existed, the new test collection failed with the expected missing-file error; no scientific case ran. After implementation, seven pure helper tests passed locally under NumPy `2.3.5`, and syntax compilation passed. The connector publication then generated two candidate commits from successive report parents, but report updates advanced `develop` before either candidate was published. Both candidates remain unreferenced and no scientific run occurred. Version `0.21.3` corrects the publication status before another attempt.
...
26. the exact runner/test blobs are frozen, but the two candidate commits created so far were not published because `develop` advanced through required report corrections first; version `0.21.3` records this chronology and prohibits scientific execution until a fresh branch-reachable commit is verified.
