...
**Scope:** Core-only research testing the owner proposal that an organized pattern may require both an internally active process (the “yeast”) and a spatial scaffold (the “mould”). This lane uses no Lina EI, symbolic memory, private data, Dynamics, OEA, external language model, or product identity model.
...
That is approximately four thousandths of one percentage point, and every measured `mu` effect was slightly negative.

The structured `kappa` scaffold produced a small local increase in energy returning to the erased sector. Its maximum recovery advantage over the mean-matched uniform map was:

```text
0.024091442639686052
```
...
- yeast `Y`: active accumulation of `mu` and its `1 + mu` feedback multiplier;
- mould `S`: structured static `kappa` map;
- damage: erase the right-hand 90-degree ring sector from `psi` and `phi`;
- retained internal trace: preserve `mu` during damage;
- mould removal: replace structured `kappa` with a mean-matched uniform map.
...
Final statements must be classified as:
...
- `mu` is read through `1 + mu`; `use_mu` controls updating it.
...
|---|---:|---:|---|
| `YS00` | absent | absent | uniform-map passive null |
| `Y1S0` | present | absent | `mu` without form |
| `Y0S1` | absent | present | form without `mu` |
| `Y1S1` | present | present | tested combination |
...
Structured and uniform maps had the same mean to floating-point precision. All structured values remained within `(0,1]`.

Damage mask:

```text
ring > 0.20 and abs(theta) <= pi/4
```
...
- formation steps `{20,60}`;
- `mu_eta {0.005,0.020}`;
- `kappa` contrast `{0.25,0.40}`;
- repair steps `40`;
- post-removal steps `40`;
- `mu_rho=0.0001`;
...
1. `Y1S1` recovery at least `0.10` above all three competitors;
2. outside-mask NRMSE no more than `0.10` worse than the best single factor;
3. total `psi` energy no more than `25%` above its twin;
4. `max(abs(psi)) < 10`;
5. no non-finite state, fail-safe reset, or invalid `kappa`.
...
```json
{
  "schema": "lineum-core-active-mu-kappa-repair-summary-v1",
  "result_sha256": "4ea006e0c29f887fd4eb46472512726909e751cb9bd3980daf5f4ec88c569de6",
  "script_sha256": "fad38b738b4f2eb68a2fe5d7732e131a599d813e48d7cd79699a3f4865440a45",
  "supportive_cells": [],
  "combined_supported_in_any_cell": false,
  "max_absolute_mu_recovery_effect": 0.00004216379362752265,
  "max_absolute_kappa_recovery_effect": 0.024091442639686052,
  "environment": {
    "python": "3.13.5",
    "numpy": "2.3.5",
...
- uniform-map lanes: approximately `0.1316–0.1347`;
- structured-map lanes: approximately `0.1464–0.1653`.

Thus the local structured-map benefit came with more deviation outside the erased sector.

Damage-region energy retention over the 40-step post-removal continuation:
...
- `mu` as a repair recipe;
- synergy between active `mu` and structured `kappa`;
- autonomous regrowth;
...
1. an active closed resource cycle such as the `epsilon` extension;
...
Recorded the combined yeast-and-mould hypothesis.

### 2026-07-30 — protocol creation

Created the active report before mechanism retrieval or implementation.

### 2026-07-30 — whitepaper handoff designation

Designated this report as the later whitepaper evidence source and added claim-promotion categories.

### 2026-07-30 — repository retrieval

Retrieved the current manifest, active Core law, equation history, active-cycle extensions, Relic Foam experiments, and ontology/deprecation documents. Separated implementation facts, experimental variants, and unsupported analogies.

### 2026-07-30 — first-pilot selection and freeze

Selected active `mu × kappa` as the smallest current-Core test and committed the complete eight-cell protocol, metrics, controls, and thresholds before execution.
