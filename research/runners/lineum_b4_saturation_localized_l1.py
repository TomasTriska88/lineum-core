...
def geometry() -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    row, column = np.indices((GRID_SIZE, GRID_SIZE), dtype=float)
    drow = row - center
    dcolumn = column - center
    radius = np.sqrt(drow * drow + dcolumn * dcolumn)
    theta = np.arctan2(dcolumn, drow)
    return row, column, radius, theta


ROW, COLUMN, RADIUS, THETA = geometry()
RADIAL_BINS = np.floor(RADIUS).astype(int)
RADIAL_BIN_COUNT = int(RADIAL_BINS.max()) + 1
_GAUSSIAN_RAW = np.exp(-(RADIUS * RADIUS) / (2.0 * SIGMA * SIGMA))
...
def diffuse(
    ...
    """Batched equivalent of the frozen two-dimensional NumPy diffusion path."""
...
def spatial_metrics(psi: np.ndarray, phi: np.ndarray) -> dict[str, Any]:
    ...
    if total > 0.0 and math.isfinite(total):
        center = (GRID_SIZE - 1) / 2.0
        center_row = float((energy * ROW).sum() / total)
        center_column = float((energy * COLUMN).sum() / total)
        displacement = float(
            math.hypot(center_row - center, center_column - center)
        )
        ...
        anisotropy = float(abs(np.sum(energy * np.exp(4j * THETA))) / total)
    else:
...
def build_lane_arrays(
    specs: list[tuple[Lane, float]],
) -> dict[str, np.ndarray]:
    return {
        "use_guard": np.array(
            [lane.use_psi_guard for lane, _ in specs], dtype=bool
        )[:, None, None],
        "use_phi_cap": np.array(
            [lane.use_phi_cap for lane, _ in specs], dtype=bool
        )[:, None, None],
        "use_tanh": np.array(
            [lane.use_tanh for lane, _ in specs], dtype=bool
        )[:, None, None],
        "use_denominator": np.array(
            [lane.use_interaction_denominator for lane, _ in specs], dtype=bool
        )[:, None, None],
        "use_mode_coupling": np.array(
            [lane.use_mode_coupling for lane, _ in specs], dtype=bool
        )[:, None, None],
        "dissipation": np.array(
            [lane.effective_dissipation_rate for lane, _ in specs], dtype=float
        )[:, None, None],
    }


def advance_batch_one_step(
    ...
    """Advance the frozen deterministic research update by one step."""
    if active is None:
    ...
    # Production NumPy semantics name axis 0 as x and axis 1 as y.
    # For the batched arrays these are axes 1 and 2 respectively.
    ...
    current_psi = current_psi + diffuse(
        current_psi, kappa, 0.05, stencil
    ) * kappa * DT
    ...
    current_phi = current_phi + 0.05 * diffuse(
        current_phi, kappa, 0.05, stencil
    )
...
def _mark_failures(
    ...
    phi: np.ndarray,
    active: np.ndarray,
    last_finite_psi: np.ndarray,
    last_finite_phi: np.ndarray,
    failure_step: np.ndarray,
    failure_stage: list[str | None],
    ...
    newly_bad = active & ~finite_rows
    for index in np.flatnonzero(newly_bad):
        failure_step[index] = step_number
        failure_stage[index] = stage
        failure_reason[index] = "nonfinite_state"
    if newly_bad.any():
        psi[newly_bad] = last_finite_psi[newly_bad]
        phi[newly_bad] = last_finite_phi[newly_bad]
        active[newly_bad] = False
    good = active & finite_rows
    last_finite_psi[good] = psi[good]
    last_finite_phi[good] = phi[good]
    return psi, phi, newly_bad
...
def run_stencil(stencil: str) -> list[dict[str, Any]]:
    ...
    lane_arrays = build_lane_arrays(specs)
    ...
    with np.errstate(all="ignore"):
        for step_index in range(STEPS):
            ...
            _progress(stencil, "primary", step_index + 1, STEPS, started)
    ...
    perturb_inner = RADIUS <= 2.0
    perturb_annulus = (RADIUS >= 3.0) & (RADIUS <= 5.0)
    psi[:, perturb_inner] *= np.where(active[:, None], 1.5, 1.0)
    psi[:, perturb_annulus] *= np.where(active[:, None], 0.5, 1.0)
    last_finite_psi[active] = psi[active]
    ...
    recovery_steps_completed = np.zeros(batch_size, dtype=int)
    ...
    with np.errstate(all="ignore"):
        for recovery_index in range(RECOVERY_STEPS):
            ...
            psi, phi, newly_bad = _mark_failures(
                psi,
                phi,
                active,
                last_finite_psi,
    ...
    for index, (lane, phi0) in enumerate(specs):
        ...
        reset_free = resets[index] + recovery_resets[index] == 0
        psi_recovery = bool(
            finite
            and reset_free
        ...
        phi_one_sided_stationary = bool(
            phi_slopes[index] <= phi_stationary_threshold
        )
        phi_abs_stationary = bool(
            abs(phi_slopes[index]) <= phi_stationary_threshold
        )
        full_recovery = bool(
            psi_recovery
        ...
        rows.append(
            {
                ...
                "steps_completed": (
                    STEPS
                    if failure_stage[index] != "primary"
                    else int(failure_step[index] - 1)
                ),
                "recovery_steps_completed": (
                    int(recovery_steps_completed[index])
                    if failure_stage[index] != "primary"
                    else 0
                ),
                ...
                "tail": {
                    ...
                    "phi_abs_slope_per_step": float(abs(phi_slopes[index])),
                    ...
                    "phi_absolute_stationary_secondary": phi_abs_stationary,
                },
                "events": {
                    ...
                    "last_finite_state_preserved": bool(
                        failure_reason[index] is not None
                    ),
                },
                "recovery": {
                    ...
                    "secondary_nontrivial_localized_pre_state": (
                        secondary_nontrivial
                    ),
                    "secondary_nontrivial_and_psi_recovery": bool(
                        secondary_nontrivial and psi_recovery
                    ),
                },
                "boundedness": {
                    "finite": finite,
                    ...
                    "psi_cap_free": bool(
                        psi_cap_hits[index] + recovery_psi_cap_hits[index] == 0
                    ),
                    "phi_cap_free": bool(
                        phi_cap_hits[index] + recovery_phi_cap_hits[index] == 0
                    ),
                },
            }
        )
...
def _pair_status(row: dict[str, Any]) -> tuple[bool, ...]:
    return (
        row["boundedness"]["finite"],
        row["boundedness"]["reset_free"],
        row["boundedness"]["psi_cap_free"],
        row["boundedness"]["phi_cap_free"],
        row["recovery"]["localized_psi_recovery"],
        row["recovery"]["localized_full_state_recovery"],
    )


def classify(rows: list[dict[str, Any]]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for lane in LANES:
        for phi0 in PHI0_VALUES:
            ...
            same_boundedness_and_recovery = _pair_status(lap4) == _pair_status(lap8)
            radial_advantage = abs(
                lap4["recovery"]["energy_radial_profile_l2_error"]
                - lap8["recovery"]["energy_radial_profile_l2_error"]
            )
            anisotropy_advantage = abs(
                lap4["pre_perturbation"]["anisotropy4"]
                - lap8["pre_perturbation"]["anisotropy4"]
            )
            material_advantage = bool(
                math.isfinite(radial_advantage)
                and math.isfinite(anisotropy_advantage)
                and (radial_advantage > 0.10 or anisotropy_advantage > 0.10)
            )
            pairs.append(
                {
                    "lane": lane.name,
                    "phi0": phi0,
                    "lap4_status": list(_pair_status(lap4)),
                    "lap8_status": list(_pair_status(lap8)),
                    "same_boundedness_and_recovery": same_boundedness_and_recovery,
                    "radial_error_absolute_difference": float(radial_advantage),
                    "anisotropy_absolute_difference": float(anisotropy_advantage),
                    "material_metric_advantage": material_advantage,
                    ...
                    "lap4_anisotropy": lap4["pre_perturbation"]["anisotropy4"],
                    "lap8_anisotropy": lap8["pre_perturbation"]["anisotropy4"],
    ...
    same_all = all(pair["same_boundedness_and_recovery"] for pair in pairs)
    any_material_advantage = any(
        pair["material_metric_advantage"] for pair in pairs
    )
    phi_resolved = all(
        row["boundedness"]["finite"]
        and row["tail"]["phi_one_sided_stationary"]
    ...
    classification = {
        ...
        "baseline_primary_psi_recoveries": sum(
            row["recovery"]["localized_psi_recovery"] for row in baseline
        ),
        "baseline_primary_full_state_recoveries": sum(
            row["recovery"]["localized_full_state_recovery"]
            for row in baseline
        ),
        "baseline_secondary_nontrivial_states": sum(
            row["recovery"]["secondary_nontrivial_localized_pre_state"]
            for row in baseline
        ),
        "baseline_secondary_nontrivial_psi_recoveries": sum(
            row["recovery"]["secondary_nontrivial_and_psi_recovery"]
            for row in baseline
        ),
        ...
        "stencil_not_decisive_in_l1": bool(
            same_all and not any_material_advantage
        ),
        "all_pair_boundedness_and_recovery_statuses_equal": bool(same_all),
        "any_material_stencil_metric_advantage": bool(any_material_advantage),
        "development_programme_terminal_failure": False,
    }
    return classification, pairs
...
def run() -> dict[str, Any]:
    ...
    classification, pairs = classify(rows)
    lane_summary: dict[str, Any] = {}
    for lane in LANES:
        subset = [row for row in rows if row["lane"] == lane.name]
        lane_summary[lane.name] = {
            "runs": len(subset),
            "finite_runs": sum(row["boundedness"]["finite"] for row in subset),
            "primary_psi_recoveries": sum(
                row["recovery"]["localized_psi_recovery"] for row in subset
            ),
            "primary_full_recoveries": sum(
                row["recovery"]["localized_full_state_recovery"]
                for row in subset
            ),
            "secondary_nontrivial_states": sum(
                row["recovery"]["secondary_nontrivial_localized_pre_state"]
                for row in subset
            ),
            "secondary_nontrivial_psi_recoveries": sum(
                row["recovery"]["secondary_nontrivial_and_psi_recovery"]
                for row in subset
            ),
            "reset_runs": sum(
            ...
            "failed_runs": sum(
                row["events"]["failure_reason"] is not None for row in subset
            ),
    ...
    return {
        "schema": "lineum-b4-saturation-localized-l1/2",
        "source": {
        ...
        "protocol": {
            ...
            "sigma_cells": SIGMA,
            "gaussian_peak": float(np.abs(GAUSSIAN).max()),
            "gaussian_center": (GRID_SIZE - 1) / 2.0,
            ...
            "primary_recovery_gates": (
                "the preregistered energy, radial-profile, half-radius, centre, "
                "phi-profile, cap, reset, finite-state, and one-sided phi-slope gates"
            ),
            "secondary_nontrivial_diagnostic": (
                "pre energy >=1% initial and >=50% within radius 6; retained as "
                "secondary only because it was not part of the preregistered primary gates"
            ),
            "failure_handling": (
                "no unregistered finite-magnitude fail-fast threshold; first non-finite "
                "step and reason are recorded and the last finite state is preserved"
            ),
            "phi_slope_policy": (
                "one-sided preregistered slope remains primary; absolute slope is a "
                "secondary diagnostic"
            ),
        },
        "fidelity_corrections": {
            "gaussian_peak_normalized_to_one": True,
            "production_gradient_axis_order_preserved": True,
            "nontrivial_gate_demoted_to_secondary": True,
            "unregistered_large_value_failfast_removed": True,
            "last_finite_failure_state_preserved": True,
            "stencil_decision_compares_all_boundedness_and_recovery_statuses": True,
            "one_sided_phi_slope_gate_preserved": True,
            "absolute_phi_slope_added_as_secondary": True,
            "physical_parameters_or_primary_thresholds_changed": False,
        },
