...
- **Fundamental Requirement:** To preserve localized phase scaffolding while capping thermodynamic growth, the Eq-11 formulation most plausibly requires **external degrees of freedom**. Among the surviving external-degree-of-freedom candidates, an autonomous secondary $\Phi$ field is currently the least artifact-prone and most physically plausible next branch.
   ...
   - **Particle-First Closure (Program Suspended):** After systematically eliminating eight distinct families of physical boundaries—including dual-field buffers, biharmonic metrics, dimensional orthogonal venting, operator-split solvers, and pure phase topology—the Eq-11 architecture natively lacks the mathematical capacity to forge a closed, stable N=1 particle. The deeply baked structural deficit of reconciling non-linear fluid growth with continuous phase limits marks current 2D Eq-11 as a non-viable framework for isolated particle-genesis. The particle-first program is formally paused. Future research must exclusively pivot to identifying what continuous, macroscopic, system-wide phase phenomena Eq-11 *natively* excels at governing, rather than artificially forcing the architecture into finite structural closures.
